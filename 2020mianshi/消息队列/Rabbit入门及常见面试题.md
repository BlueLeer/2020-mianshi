...
也就是消息队列，用来保存消息只到发送给消费者。它是消息的容器，也是消息的终点。一个消息可以投入一个或者多个队列，消息一直在队列里面，等待消费者连接到这个队列将其取走。
...
* 工作队列
  ...
  一般生产者会耗费费力的将消息往队列里面发，但是消费者消费消息的时候可能要做一些逻辑处理，这样处理的速度比发送的速度慢很多的话，就会导致消息队列里面的消息产生积压。因此，有时候需要多个消费者来消费消息。按照分发策略的不同，可以分为以下两种：
...
### RabbitMQ之消息确认机制（ACK机制）

在RabbitMQ中我们可以通过持久化来解决因服务器异常重启后导致消息的丢失问题，也就是说消息到达服务器以后，放在队列里面的消息会持久化到硬盘当中，此时消费者还没来得及消费，而RabbitMQ服务器挂掉了，重启以后，消息没有丢，消费者可以继续消费。

除此之外还会遇到一个问题：生产者将消息发送出去以后，消息到底有没有正确的到达RabbitMQ服务器呢，因为在发消息的中途需要网络传输，这个过程很容易出现问题。默认情况下，消息生产者是不知道有没有正确到达RabbitMQ服务器的（因为RabbitMQ服务器不会反馈给生产者任何信息），这个时候导致的问题是：消息在发送到达服务器之前丢失，即使我们设置了持久化也是解决不了这种问题的，因为消息根本就没有达到RabbitMQ服务器。此时Rabbit为我们提供了两种解决方案：
