...
![Java内存模型](C:\Users\wangle\Desktop\2020mianshi\Java内存模型.jpg)
...
- **write（写入）**：作用于主内存的变量，它把store操作从工作内存中一个变量的值传送到主内存的变量中。
  ...
  ![Java主内存和工作内存间交互](C:\Users\wangle\Desktop\2020mianshi\Java主内存和工作内存之间交互.png)
...
它的实现原理是：ThreadLocal中有个静态内部类ThreadLocalMap，每个线程都有自己独立的ThreadLocalMap实例，当set值时，会获取当前线程的map，然后以ThreadLocal实例为key，值为value，放入到map中去
